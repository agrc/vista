...
identity = arcpy.Identity_analysis(residence_points, counties, identity)

print('search cursoring')
with arcpy.da.SearchCursor(identity, [COUNTY_ID_fld, 'COUNTYNBR', RESIDENCE_ID_fld]) as cur:
    i = 0
    for row in cur:
        if row[0] != int(row[1]):
            i = i + 1
