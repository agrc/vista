define([
    ...
    return {
        ...
        showVistaPoints: function () {
            ...
            var title = '${House_Number} ${Direction_Prefix} ${Street} ${Street_Type} ${Unit_Number} ${Unit_Type}';
            ...
            xhr(url, {
                ...
                var graphics = addresses.map(function (row) {
                    var webMercator = project([row.X, row.Y]);
                    return new Graphic(new Point(webMercator, new SpatialReference(3857)), config.vistaSymbol, row, infoTemplate);
                });
