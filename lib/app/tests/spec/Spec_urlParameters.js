require([
    ...
    describe('app/UrlParameters', function () {
        ...
        it('makes the request if required parameter (displayMode) is present', function () {
            spyOn(urlParameters, 'showVistaPoints');
            var url = 'http://test.com/?db=p&residenceID=resid';
            urlParameters.init(url, app);
            mapLoadedDef.resolve();

            expect(urlParameters.showVistaPoints).not.toHaveBeenCalled();

            url = 'http://test.com/?db=p&residenceID=resid&displayMode=blah';
            urlParameters.init(url, app);
            mapLoadedDef.resolve();

            expect(urlParameters.showVistaPoints).toHaveBeenCalled();
        });
